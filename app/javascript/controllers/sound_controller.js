...
export default class extends Controller {
    ...
    connect() {
        ...
        if (this.hasVolumeTarget) {
            this.audioTarget.volume = $(this.volumeTarget).val() / 100;
            $(this.volumeDisplayTarget).text(`${$(this.volumeTarget).val()}%`);
            $(this.volumeTarget).on("input", this.update_volume.bind(this));
        }
